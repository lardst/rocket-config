...
(function () {
    ...
    function loadConfig(configFilePaths = []) {
        ...
        for (i in configFilePaths) {
            ...
            try {
                configFileContents = fs.readFileSync(path.join(thisFile.dir, thisFile.base), 'utf-8');
                parsedData = JSON.parse(cleanJSON(configFileContents));
    ...
    class RocketConfig {
        constructor (configRelativePath, optionsOverride) {
            ...
            for (domain in compiledConfig) {
                if (keyCount(compiledConfig) === 1) {
                    for (data in compiledConfig[domain].data) {
                        Object.defineProperty(this, data, {
                            value       : compiledConfig[domain].data[data],
                            enumerable  : false
                        });
                    }
                } else {
